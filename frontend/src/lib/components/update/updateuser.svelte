<script lang="ts">
	...
	async function fetchData() {
		...
		if (!id) return;
		try {
			const url = `http://127.0.0.1:8000/api/update/${kategori}/${id}/`;
...
<div class="min-h-screen bg-gray-50 p-6">
	<div class="mx-auto max-w-2xl rounded-xl bg-white p-8 shadow-sm">
		<div class="mb-6">
			...
			<p class="mt-1 text-sm text-gray-500">Silakan isi formulir di bawah untuk memperbarui data {kategori}</p>
		</div>
		...
		{:else}
			...
			<form onsubmit={updateUser} class="space-y-5">
				...
				<div class="space-y-2">
					<label for="kartu_pelajar_input" class="block text-sm font-medium text-gray-700">Kartu Pelajar</label>
						...
						<div class="flex-1">
							{#if imagePreview}
								<div class="h-32 w-full relative">
									<img src={imagePreview} alt="Kartu Pelajar" class="h-32 w-full object-contain border rounded" />
								</div>
							{:else}
					...
					<p class="text-xs text-gray-500 mt-1">Unggah gambar kartu pelajar baru jika ingin mengganti yang lama</p>
				</div>
			{:else if kategori === 'Guru'}
				<div class="">
				...
				<div class="">
					<label for="password_guru" class="block text-sm font-medium text-gray-700">Password (kosongkan jika tidak ingin mengganti)</label>
					<input
						id="password_guru"
						type="password"
						bind:value={password}
			...
			{:else if kategori === 'Pegawai'}
				<div class="">
					<label for="id_pegawai_field" class="block text-sm font-medium text-gray-700">ID Pegawai </label>
					<input
						id="id_pegawai_field"
						type="text"
				...
				<div class="">
					<label for="nama_pegawai_field" class="block text-sm font-medium text-gray-700">Nama</label>
					<input
				...
				<div class="">
					<label for="bidang_pegawai" class="block text-sm font-medium text-gray-700">Bidang</label>
					<input
						id="bidang_pegawai"
						type="text"
						bind:value={bidang}
				...
				<div class="">
					<label for="password_pegawai" class="block text-sm font-medium text-gray-700">Password (kosongkan jika tidak ingin mengganti)</label>
					<input
						id="password_pegawai"
						type="password"
						bind:value={password}
